...
const outputPath = resolve(
  process.env.STATION_NATIVE_GIT_BARE_SINGLEFLIGHT_OUTPUT ??
    ".dev-state/performance/quick-session/native-git-bare-singleflight-comparison.real.json",
);
...
const report = {
  schemaVersion: 1,
  benchmark: "station-quick-session-native-git-bare-singleflight-comparison",
  ...
  keepThresholds: {
    burst5MedianImprovementFraction: 0.08,
    burst5P95ImprovementFraction: 0.08,
    burst20MedianImprovementFraction: 0.08,
    burst20P95ImprovementFraction: 0.08,
    singleMaximumP95RegressionFraction: 0.15,
  },
  settledEvictionCheck: await settledEvictionCheck(),
  allSafe: false,
...
for (const scenario of report.scenarios) {
  const independentRuns = scenario.pairs.map((pair) => pair.independent);
  const singleflightRuns = scenario.pairs.map((pair) => pair.singleflight);
  scenario.independent = summarize(independentRuns);
  scenario.singleflight = summarize(singleflightRuns);
  scenario.medianImprovementFraction = improvement(
    scenario.independent.wallMs.median,
    scenario.singleflight.wallMs.median,
  );
  scenario.p95ImprovementFraction = improvement(
    scenario.independent.wallMs.p95,
    scenario.singleflight.wallMs.p95,
  );
  scenario.probeCountReduced =
    scenario.burstSize === 1 ||
    scenario.pairs.every(
      (pair) => pair.singleflight.probes.length < pair.independent.probes.length,
    );
  scenario.allSafe = [...independentRuns, ...singleflightRuns].every((runResult) => runResult.safe);
}
...
report.allSafe =
  report.settledEvictionCheck.safe &&
  report.scenarios.every((scenario) => scenario.allSafe && scenario.probeCountReduced) &&
  report.repetitions.every((repetition) => repetition.temporaryRootRemoved);
report.thresholdsPassed =
  single.p95ImprovementFraction >= -report.keepThresholds.singleMaximumP95RegressionFraction &&
...
process.stdout.write(`[native Git bare singleflight comparison] ${outputPath}\n`);
if (!report.allSafe || !report.thresholdsPassed) process.exitCode = 1;

async function runRepetition(repetition) {
  const temporaryRoot = await mkdtemp(join(tmpdir(), "station-native-bare-singleflight-"));
    ...
    for (const [scenarioIndex, burstSize] of burstSizes.entries()) {
      const order =
        (repetition + scenarioIndex) % 2 === 0
          ? ["independent", "singleflight"]
          : ["singleflight", "independent"];
      const strategies = {};
      ...
      scenarioNamed(burstSize).pairs.push({
        ...
        singleflight: strategies.singleflight,
      });
    }
  } finally {
...
async function initializeRepository(root, shapeRoot) {
  ...
  if (ordinaryInventory.length !== initialWorktrees) {
    throw new Error(`Bare singleflight fixture expected ${initialWorktrees} worktrees.`);
  }
...
async function runStrategy(input) {
  const commands = Array.from({ length: input.burstSize }, (_, index) => ({
    branch: `bare-${input.strategy}-${input.repetition}-${input.burstSize}-${index}`,
    path: join(
      input.createdRoot,
      `bare-${input.strategy}-${input.repetition}-${input.burstSize}-${index}`,
    ),
  }));
  const probes = [];
  let nextProbeId = 0;
  const executeProbe = async () => {
    ...
    nextProbeId += 1;
    const probe = await runBareProbe(input.root, probeId);
    probes.push(probe);
    return probe;
  };
  const probe =
    input.strategy === "singleflight"
      ? createSettledEvictingSingleflight(executeProbe)
      : executeProbe;
  ...
  const results = await mapBounded(commands, maxConcurrent, async (command) => {
    ...
    try {
      return await createAndVerify({ ...input, ...command, probe });
    } finally {
      active -= 1;
    }
  });
  ...
  const probeIds = new Set(probes.map((bareProbe) => bareProbe.id));
  const safe =
    results.every((result) => result.safe && result.probeSafe && probeIds.has(result.probeId)) &&
    exactInventory &&
    probes.every((bareProbe) => bareProbe.safe) &&
  ...
  return {
    ...
    probes: probes.map((bareProbe) => ({
      id: bareProbe.id,
      ms: bareProbe.ms,
      safe: bareProbe.safe,
      code: bareProbe.code,
      stderr: redact(bareProbe.stderr, dirname(input.root)),
    })),
    commands: results.map((result) => ({
      safe: result.safe,
      probeId: result.probeId,
      probeSafe: result.probeSafe,
...
async function runBareProbe(root, id) {
...
async function createAndVerify(input) {
  const bareProbe = await input.probe();
  if (!bareProbe.safe) {
    return failedCommand(bareProbe, bareProbe.id, true, 0, 0);
  }
  ...
  if (mutation.code !== 0) {
    return failedCommand(mutation, bareProbe.id, bareProbe.safe, mutation.ms, 0);
  }
  ...
  return {
    ...
    probeId: bareProbe.id,
    probeSafe: bareProbe.safe,
...
function failedCommand(command, probeId, probeSafe, mutationMs, verificationMs) {
  return {
    safe: false,
    probeId,
    probeSafe,
...
function createSettledEvictingSingleflight(execute) {
  let pending;
  return () => {
...
async function settledEvictionCheck() {
  let executions = 0;
  const probe = createSettledEvictingSingleflight(async () => {
    executions += 1;
    await Promise.resolve();
    return executions;
  });
  const simultaneous = await Promise.all([probe(), probe(), probe(), probe()]);
  const afterSettlement = await probe();
  return {
    safe: executions === 2 && simultaneous.every((value) => value === 1) && afterSettlement === 2,
    executions,
    simultaneous,
    afterSettlement,
  };
...
function scenarioNamed(burstSize) {
  ...
  if (scenario === undefined) throw new Error(`Missing bare singleflight burst ${burstSize}.`);
  return scenario;
}
...
function summarize(runs) {
  return {
    ...
    probeExecutions: distribution(runs.map((runResult) => runResult.probes.length)),
    probeMs: distribution(runs.flatMap((runResult) => runResult.probes.map((probe) => probe.ms))),
