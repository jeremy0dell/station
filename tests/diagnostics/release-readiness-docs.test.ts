...
describe("release readiness docs", () => {
  ...
  it("keeps the agent-led install prompt in one canonical location", async () => {
    ...
    expect(prompt).toContain(
      "https://github.com/jeremy0dell/station/releases/download/v0.0.0-pre-alpha.14.6/install.sh",
    );
    expect(prompt).toContain("v0.0.0-pre-alpha.14.6");
    ...
    expect(install).not.toContain("Install experimental Station v0.0.0-pre-alpha.14.6");
  });
  ...
  it("documents and enforces the public exact-tag binary release contract", async () => {
    ...
    const exactVersion = "v0.0.0-pre-alpha.14.6";
    ...
    expect(packageJson.version).toBe("0.0.0-pre-alpha.14.6");
