...
describe("release readiness docs", () => {
  ...
  it("keeps the agent-led install prompt in one canonical location", async () => {
    ...
    expect(prompt).toContain("v0.0.0-pre-alpha.14.7");
    ...
    expect(install).not.toContain("Install experimental Station v0.0.0-pre-alpha.14.7");
  });
  ...
  it("documents and enforces the public exact-tag binary release contract", async () => {
    ...
    const exactVersion = "v0.0.0-pre-alpha.14.7";
    ...
    expect(packageJson.version).toBe("0.0.0-pre-alpha.14.7");
