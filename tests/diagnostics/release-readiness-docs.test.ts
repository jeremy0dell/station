...
describe("release readiness docs", () => {
  ...
  it("provides an agent-led binary install and setup validation prompt", async () => {
    ...
    for (const document of [readme, install]) {
      ...
      expect(prompt).toContain("v0.0.0-pre-alpha.2");
  ...
  it("documents and enforces the public exact-tag binary release contract", async () => {
    ...
    const exactVersion = "v0.0.0-pre-alpha.2";
    ...
    expect(development).toContain("accepted-release-candidate-0.0.0-pre-alpha.2");
    ...
    expect(installSmoke).toContain('const releaseTag = "v0.0.0-pre-alpha.2"');
    ...
    expect(packageJson.version).toBe("0.0.0-pre-alpha.2");
