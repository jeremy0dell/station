...
The current test is a conservative launch-scaffolding lane. It creates a temporary git worktree, starts a unique tmux session, launches a Pi-shaped wrapper through `createPiHarnessProvider`, verifies the launched argv includes `--extension <dist/piExtension.js>`, and reconciles a provider-neutral Pi harness run with low-confidence `unknown` status.
