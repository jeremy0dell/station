...
- `codex-approval-notification.test.ts` starts the real interactive Codex TUI through the production Host PTY table with `on-request` approval, asks it to write outside its sandbox, and proves Host emits a content-free OSC 9 notification while the approval UI is open and before the write occurs. It maps the stable Host ID and timestamp through the Codex hook adapter, asserts `needs_attention` with `tool_approval`, and cancels without creating the file.
