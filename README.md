...
`v0.0.0-pre-alpha.14.8`, with one command:

```sh
curl --disable -fsSL https://github.com/jeremy0dell/station/releases/download/v0.0.0-pre-alpha.14.8/install.sh | sh
```
...
```text
Install experimental Station v0.0.0-pre-alpha.14.8 and validate setup on this machine.

Safety and scope:
- Do not clone the repository or build from source. Use only
  `https://github.com/jeremy0dell/station/releases/download/v0.0.0-pre-alpha.14.8/install.sh`.
...
Station `v0.0.0-pre-alpha.14.8` is an experimental pre-alpha. User-facing commands,
