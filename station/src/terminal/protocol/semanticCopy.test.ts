...
describe("Station semantic copy protocol", () => {
  it("encodes the exact bounded v1 markers", () => {
    expect(HARD_BOUNDARY_MARKER).toBe("\x1b]6973;station-copy;1;hard\x1b\\");
  ...
  it("consumes OSC 6973 invisibly and records only a valid continuation", async () => {
    ...
    try {
      ...
      expect(state.boundaryForBufferRow("normal", 1)).toEqual({
        kind: "soft",
        leadingColumns: 2,
        separatorSpaces: 2,
      });

      await write(terminal, `\r> ${HARD_BOUNDARY_MARKER}`);
      expect(state.boundaryForBufferRow("normal", 1)).toEqual({
        kind: "hard",
        leadingColumns: 2,
      });
      ...
      expect(state.snapshot().normal).toEqual([
        { kind: "soft", row: 1, leadingColumns: 2, separatorSpaces: 3 },
      ]);
      ...
      expect(state.boundaryForBufferRow("normal", 2)).toBeUndefined();
      ...
      expect(state.boundaryForBufferRow("normal", 3)).toBeUndefined();
    } finally {
      state.dispose();
      terminal.dispose();
    }
  ...
  it("follows normal-buffer scrollback and never survives eviction", async () => {
    ...
    try {
      ...
      await write(terminal, "\r\nthree");
      expect(state.snapshot().normal).toEqual([
        { kind: "soft", row: 1, leadingColumns: 0, separatorSpaces: 0 },
      ]);
  ...
  it("supports alternate-buffer rows and round-trips both buffer snapshots", async () => {
    ...
    try {
      ...
      expect(snapshot).toEqual({
        normal: [{ kind: "soft", row: 1, leadingColumns: 0, separatorSpaces: 1 }],
        alternate: [{ kind: "soft", row: 2, leadingColumns: 2, separatorSpaces: 0 }],
      });
  ...
  it("validates restored snapshots and drops rows that no longer map", async () => {
    ...
    try {
      ...
      expect(state.snapshot()).toEqual({ normal: [], alternate: [] });
      state.restore({
        normal: [{ kind: "soft", row: 0, leadingColumns: 0, separatorSpaces: 0 }],
        alternate: [],
      });
      expect(() =>
        state.restore({
  ...
  it("moves metadata with xterm line insertion and deletion", async () => {
    ...
    try {
      state.restore({
        normal: [{ kind: "soft", row: 2, leadingColumns: 0, separatorSpaces: 1 }],
        alternate: [],
      });
  ...
  it("keeps hard boundaries hard while resize reflows native rows", async () => {
    ...
    try {
      ...
      expect(screen.viewRowCopyBoundary(2)).toBeUndefined();
    } finally {
      screen.dispose();
    }
  });
  ...
  it("keeps application metadata on its row through resize reflow", async () => {
    ...
    try {
      ...
      expect(screen.viewRowCopyBoundary(2)).toEqual({ kind: "terminal-soft" });
    } finally {
      screen.dispose();
    }
  });
