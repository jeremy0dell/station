...
export async function runHostHandoff(
  ...
  if (input.inspection.status !== "exact") return projectUnavailableHandoff(input.inspection, base);
  ...
  if (evidence.terminals.length > 0 && !stationHostTerminalsAreHandoffEligible(evidence.terminals))
    return {
      ...base,
      status: "refused",
...
function projectUnavailableHandoff(
  ...
  switch (inspection.status) {
    ...
    case "inaccessible":
      return { ...base, status: "unavailable", message: inspection.error.message };
    case "unknown":
