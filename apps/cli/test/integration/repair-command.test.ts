...
describe("repair CLI integration", () => {
  it("returns a partial inventory from default paths on a fresh install without config", async () => {
    ...
    const result = await runCli(["repair", "inventory", "--json"], {
      repairDeps: {
        observer: {
  ...
  it("does not replace an explicitly missing config with first-run defaults", async () => {
    await expect(
      runCli([
        "--config",
