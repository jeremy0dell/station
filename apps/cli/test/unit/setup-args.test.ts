...
describe("setup args", () => {
  ...
  it("validates unsupported flag combinations", () => {
    ...
    expect(() => parseSetupArgs(["--dry-run"])).toThrow(
      "stn setup --dry-run is not supported. Use: stn setup apply --dry-run.",
    );
    expect(() => parseSetupArgs(["--check"])).toThrow(
      "stn setup --check is not supported. Use: stn setup check.",
    );
    expect(() => parseSetupArgs(["--json"])).toThrow(
      "stn setup --json is not supported. Use: stn setup check --json.",
    );
    expect(() => parseSetupArgs(["--yes"])).toThrow(
      "stn setup --yes is not supported. Use: stn setup apply --yes.",
    );
    expect(() => parseSetupArgs(["--no-brew"])).toThrow(
      "stn setup --no-brew is not supported. Use: stn setup check --no-brew.",
    );
    ...
    expect(() => parseSetupArgs(["apply", "--json"])).toThrow(
      "--json is supported for stn setup check and stn setup plan.",
    );
