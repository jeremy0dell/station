...
describe("parseHostArgs", () => {
  ...
  it("rejects updater-only flags", () => {
    expect(() => parseHostArgs(["handoff", "--update-crossover"])).toThrow(/Unknown host flag/);
    expect(() => parseHostArgs(["handoff", "--replacement-required"])).toThrow(/Unknown host flag/);
  });
