...
describe("session command vertical slice", () => {
  ...
  it("starts fresh under the retained session identity without changing its Group", async () => {
    ...
    await expect(fixture.persistence.listSessions()).resolves.toEqual([sessionBefore]);
    expect(
      (await fixture.persistence.listEvents({ commandId: receipt.commandId })).map(
        (event) => event.type,
      ),
