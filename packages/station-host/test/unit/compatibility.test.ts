...
describe("classifyHostCompatibility", () => {
  it("treats the version 7 request shape as incompatible with version 8", () => {
    expect(HOST_PROTOCOL_VERSION).toBe(8);
    expect(
      classifyHostCompatibility(
        { ok: true, protocolVersion: 7, buildVersion: "build-current" },
        "build-current",
      ),
