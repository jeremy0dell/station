...
function handleSectionListKey(
  ...
  if (key.escape === true || key.leftArrow === true) return { state: cancelGroupSettings(state) };
  const direct = GROUP_SETTINGS_ITEMS.find((item) => item.shortcut === key.input)?.id;
  if (direct !== undefined) return { state: selectGroupSettingsSection(state, direct, "list") };
  if (key.upArrow === true || key.downArrow === true) {
    ...
    return {
      state: section === undefined ? state : selectGroupSettingsSection(state, section, "list"),
    };
  }
