 ...
 * ADAPTER
 *
 * Owns the generated `station-agent-state.js` plugin body and its rendering
 * from install-time values. The body is plain JavaScript with no backticks or
 * `${` interpolation so the surrounding template literal needs no escaping;
 * install-time values are `__STATION_*__` placeholder tokens that
 * `renderStationOpenCodePlugin` replaces with JSON-encoded literals.
 */
...
const STATION_OPENCODE_PLUGIN_BODY = `import { spawn, spawnSync } from "node:child_process";
...
export function renderStationOpenCodePlugin(input: StationOpenCodePluginScriptInput): string {
  return STATION_OPENCODE_PLUGIN_BODY.replaceAll(
    STATION_OPENCODE_PLUGIN_TOKENS.socketPath,
    JSON.stringify(input.observerSocketPath),
  )
