...
- Host retains complete transformed output and ordered resize transitions within a 256 KiB replay budget, plus a bounded Unicode-11 headless terminal model from the first byte. Attach returns exact ordered raw replay while complete; after eviction it returns self-contained semantic restoration VT at current geometry, with live output and resize ordered behind the same capture barrier.
