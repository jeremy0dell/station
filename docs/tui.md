...
- Dashboard row click, focused Enter, a bare `1-9/a-z` shortcut, and a submitted
  backtick-prefixed logical shortcut are the explicit authorization to relaunch a
  proven-exited managed pane. Pane clicks, pane
...
- Group ordering is the renderer-local union `"groups-first" | "alphabetical-interleaved"`. Groups-first is the default and places alphabetized Group blocks before project-root sessions. Alphabetical interleaving compares Group names with root-session display titles while keeping every Group block intact. Both modes retain canonical arrays, collapse and filter state, and continuous logical shortcuts assigned only to rendered sessions; no user-facing config key selects the mode.
- The dashboard footer selects one explicit loading, regular, filter, or command component from a semantic footer model. The root footer owns only that exhaustive family dispatch; each component owns its own presentation and receives no models belonging to another family. Dashboard session shortcuts use a lowercase bijective base-35 sequence over `1-9/a-z`: `1-9`, `a-z`, `11-1z`, `21-2z`, and so on through `zzz`, capped at 44,135 shortcut-bearing sessions. Zero never appears. Bare `1-9/a-z` invokes the first 35 codes immediately, leaving uppercase keys for commands. Backtick opens a timeout-free command bar that preserves case: one registered uppercase command such as `X` or one lowercase session code runs on `Enter`; `Backspace` edits, and `Esc` or a second backtick closes. `?` opens Help without discarding the current draft; the command bar remains visible beneath Help in a dimmed, cursor-free suspended state, and closing Help resumes it exactly where it left off. The bar composes stable `SHORTCUT`, `INPUT`, preview, flexible-space, and control zones; the preview carries the `SESSION`/`COMMAND`/`NO MATCH` interpretation while controls stay pinned to the right edge. Narrower widths simplify those same zones before hiding the preview or nonessential control descriptions. Filtering, collapse, and ordering rebuild the projected registry, while scrolling does not renumber it. After Remove, Rename, Move, or Fork opens its existing session chooser, that chooser keeps the same global codes and accepts the backtick prefix for extended targets. Sessions beyond `zzz` remain reachable by cursor or pointer but have no shortcut code.
...
highlighting. Every rendered row keeps its current order, logical shortcut, collapse visibility, and viewport
...
visible, selectable canonical-session traversal with `▸` as its keyboard cursor; global logical shortcuts and Enter
...
a one-key session shortcut. Gaps and optimistic create rows remain non-focusable and receive no session shortcut.
