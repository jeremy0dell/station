...
- New Session and Fork Session expose **Name** as the editable product concept. The initial name matches a generated, path-safe Git branch, but later name edits may contain spaces and punctuation and never mutate that hidden branch identity. Quick Session uses its generated branch as the default name.
