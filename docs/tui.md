...
- `station/src/terminal/` is the app-local PTY boundary (VT parser/screen model under `terminal/vt/`); `station/src/host/` is the PTY-host client for warm/cold reattach. A managed pane resolves exactly one live Host target and retains its session-bound `{ terminalTargetId, ptyId, ptyInstanceId, sessionId }` identity through reconnect, negotiated handoff, and crash adoption. Duplicate targets, stale identities, session reassignment, and mismatched attach acknowledgements fail as attachment unavailable without spawning a local replacement. `terminal/protocol/` owns the typed VT syntax categories, complete command identifiers and sequence constants, domain values, and pure protocol reducers consumed by both paths. Production composes explicit byte templates from that vocabulary rather than embedding ESC/BEL literals or mode numbers; raw sequences remain valid in protocol byte tables, tests, conformance cases, and parser-local regex syntax.
