...
- Host retains complete transformed output and ordered resize transitions within a 256 KiB replay budget, plus a bounded Unicode-11 headless xterm model from the first byte. Attach returns exact ordered raw replay while complete; after eviction it uses xterm's serializer plus a small Station-specific mode supplement. Capture fails closed and retries between xterm parser boundaries, with live output and resize ordered behind the same barrier.
- Attachment-unavailable state is not process exit: snapshot/version/reconnect failures stop pane input and resize forwarding and show `attachment unavailable`, while only proven Host absence, an exited acknowledgement, or an exit frame reaches the pane-exit lifecycle.
