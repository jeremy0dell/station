...
- Use `@station/dashboard-core/text` for terminal-cell measurement, grapheme-safe clipping, and truncation. JavaScript string indices and lengths are not terminal geometry. Compact one-cell row grids remain valid leaf presentations but do not define dashboard composition; viewport-local shortcut labels decorate a cached cross-row projection rather than triggering complete-tree width negotiation while scrolling.
...
Both renderer roots inject Station's Node folder-navigation adapter; dashboard-core owns the
chooser and polling lifecycle but no filesystem or parent-path fallback.
...
- `station/src/client/` holds Observer-backed and mock Station client composition; `attention/`, `config/`, `folderNavigation/`, and `host/` own their local effects and runtime policies. `station/src/state/` holds workspace state, layout persistence, and reconciliation (live dashboard commands dispatch through the single shared `@station/client` service).
