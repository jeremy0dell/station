...
- Host strips OSC 9 payload text before compatibility, replay, semantic state, diagnostics, and live delivery, retaining only the latest content-free notification ID and original timestamp for each live PTY. Host-attached terminals emit that edge after replay and deduplicate acknowledgement/live/reconnect delivery by ID. Local fallback screens expose the same message-free callback without durable retention. The pane registry keeps an unhandled edge across Observer disconnects and hot reloads, then forwards it only when the current primary Codex pane and Observer snapshot identities match exactly.
