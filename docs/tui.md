...
- Dashboard session shortcuts use a lowercase bijective base-35 sequence over `1-9/a-z`: `1-9`, `a-z`, `11-1z`, `21-2z`, and so on through `zzz`, capped at 44,135 shortcut-bearing sessions. Zero never appears. Bare `1-9/a-z` invokes the first 35 codes immediately, leaving uppercase keys for commands. Backtick opens a timeout-free command bar that preserves case: one registered uppercase command such as `X` or one lowercase session code runs on `Enter`; `Backspace` edits, and `Esc` or a second backtick closes. Filtering, collapse, and ordering rebuild the projected registry, while scrolling does not renumber it. After Remove, Rename, Move, or Fork opens its existing session chooser, that chooser keeps the same global codes and accepts the backtick prefix for extended targets. Sessions beyond `zzz` remain reachable by cursor or pointer but have no shortcut code.
