...
- Successful `agent.attach` entries in `station-host.jsonl` report `replayKind` (`raw-complete` or `semantic-truncation-recovery`), replay entry/byte counts, recorded geometry, and capture duration without terminal contents. `HOST_SNAPSHOT_FAILED` means reconstruction failed while Host still owns the PTY; confirm it remains in `host:list` and do not interpret the pane's `attachment unavailable` status as an Observer session exit.
