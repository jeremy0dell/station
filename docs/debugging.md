...
- Exact convergence revalidates the complete incumbent endpoint, health, build identity, and canonical terminal lifetimes on one non-reconnecting lifecycle connection. `beginHandoff`, `completeHandoff`, `abortHandoff`, and disconnect restoration remain bound to that physical connection's opaque owner. A successful begin receipt is correlation evidence only. The successor is accepted only after the directly spawned child is the sole canonical `lsof` holder, its pinned session proves unchanged health/endpoint, exact adoption succeeds, and a separate final inspection returns the same successor endpoint and complete target registry. The successor process form follows whoever requests convergence (`bun hostMain.ts` from source CLI, `<stn> __station-host` from a binary).
