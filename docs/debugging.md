...
- Binary-smoke runtime ownership is recorded in `rounds/*/runtime/lifecycle.jsonl`. Read the owner registration and process-start events first, then the shutdown signal, any bounded escalation, refusal or rescue event, and `runtime.cleanup.completed` counts. A complete disposable cleanup has zero active processes, sockets, and pidfiles; an incomplete or ambiguous cleanup intentionally retains its private root for the next-start rescue or manual inspection. Do not remove a retained root or signal a listed PID unless its executable, process group, start identity, and disposable role still match.
