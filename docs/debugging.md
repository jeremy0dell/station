...
- `providerReads` records each terminal provider as `complete` or `indeterminate`; an
  indeterminate entry includes its failure code.
...
live Observer whose socket is missing and a Host whose current protocol, build,
inventory, canonical lifetime identities, or path-bound endpoint cannot be
proved exactly. Non-resumable dispositions are explicit. The
...
current launcher: hook reconciliation, idempotent `observer start`, and exact
Host convergence from fresh evidence. Before an install, the Host preflight also
strictly reads durable park records and probes each live bridge without adopting
it; malformed, duplicated, unreachable, or identity-drifted parks block mutation.
An installed successor uses that successor launcher
for hook reconciliation before Observer restart. Hook failure prevents runtime
crossover. The successor Host phase accepts no-op or idle replacement only after
recovering every still-parked bridge into an independently verified exact target.
Recovery commands are the remaining ordered commands pinned to that same launcher;
`run-doctor` adds provider doctor before retry, and ownership
...
Host crossover failures additionally retain bounded phase, disposition, and
per-disposition terminal counts without exposing terminal identities.
...
- Default Host startup is display-compatible: current protocol plus exact Station display version reuses the incumbent, an idle different-display Host can replace itself, and a busy one returns `HOST_UPGRADE_BLOCKED`. Exact `STATION_HOST_HANDOFF=1` or `stn host handoff` additionally requires the immutable build identity. Under that path, the same display version with a different identity is nonexact and must use idle replacement or eligible live handoff. `host.start` in `station-host.jsonl` records protocol, display version, and immutable identity. `HOST_VERSION_INCOMPATIBLE` covers display/protocol compatibility refusal; missing, malformed, drifted, noncanonical, or incomplete exact evidence becomes a typed request/convergence refusal without `host.list` or cold-start fallback. These checks remain separate from Observer immutable-selector admission.
- The host socket defaults to `<state_dir>/run/station-host.sock` (beside `observer.sock`); override with `STATION_HOST_SOCKET_PATH`. Inspect with `bun run stn host status` or `bun run host:list` in `station/`. `stn host status` is non-mutating: it binds that configured path to inode and birth time, discovers the incumbent display build with raw strict health, then uses an incumbent-build client for strict initial health, exactly one identity-bound recovery inventory, strict final health, and the final endpoint probe. Complete current evidence preserves the existing status keys/text and exits 0. Listening but nonexact evidence exits 1; syntax or unexpected command-boundary failures exit 2. Opt into live ownership transfer separately with `bun run stn host handoff [--dry-run] [--fidelity processes|screen]` (default busy-host behavior remains refuse).
- Exact convergence revalidates the complete incumbent endpoint, health, build identity, and canonical terminal lifetimes on one non-reconnecting lifecycle connection. `beginHandoff`, `completeHandoff`, `abortHandoff`, and disconnect restoration remain bound to that physical connection's opaque owner. Abort or disconnect resumes serving only after complete restoration; a partial report retains the remaining park evidence and leaves ordinary Host operations blocked. A successful begin receipt is correlation evidence only. The successor is accepted only after the directly spawned child is the sole canonical `lsof` holder, its pinned session proves unchanged health/endpoint, exact adoption succeeds, and a separate final inspection returns the same successor endpoint and complete target registry. The successor process form follows whoever requests convergence (`bun hostMain.ts` from source CLI, `<stn> __station-host` from a binary).
- Fresh Host admission requires `/usr/sbin/lsof -t <socketPath>` on macOS or `/usr/bin/lsof -t <socketPath>` on Linux. Only a clean unsignaled status 0 with canonical positive decimal PIDs, or status 1 with exactly empty stdout, is evidence. Whitespace variants, CRLF, signs, blank lines, stderr, signals, malformed/unsafe PIDs, cancellation, deadline, and execution uncertainty all fail closed. Run `stn setup check --json` when the executable may be missing; it is a required setup fact, not an optional recovery aid.
...
- Never kill a nonexact Host or remove its socket based on display version, a receipt, or expected command evidence. Reopen with the build named by the error to finish or explicitly close live terminals, then retry; compatibility-only startup still replaces an idle different-display Host. Inspection itself grants no pin, signal, handoff, repair, or mutation authority. Canonical convergence must bind that exact inspection into a fresh command, revalidate the lifetime before release, and reports only evidence actually read from an incumbent session, target session, or independent inspection.
