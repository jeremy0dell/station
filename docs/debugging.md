...
- Successful `agent.attach` entries in `station-host.jsonl` report `replayKind` (`raw-complete` or `semantic-truncation-recovery`), replay entry/byte counts, recorded geometry, and capture duration without terminal contents. `HOST_SNAPSHOT_PENDING` is retried because later output may finish an incomplete parser sequence; `HOST_SNAPSHOT_FAILED` means reconstruction reached unsupported state while Host still owns the PTY. Confirm it remains in `host:list` and do not interpret `attachment unavailable` as an Observer session exit.
