...
durable result remains authoritative when the CLI reports a later convergence
warning. Debug export redacts `authorityId` and does not include prompt content,
...
- The host the UI dials must match both its host protocol and Station display build version. `host.start` in `station-host.jsonl` records both versions. `HOST_UPGRADE_BLOCKED` means a different display build owns live PTYs and handoff was not opted in; `HOST_VERSION_INCOMPATIBLE` means the running Host has a different display build or protocol. Missing or malformed health/build evidence instead becomes `HOST_REQUEST_FAILED` or nonexact inspection. `HOST_HANDOFF_INVALID_STATE` / `HOST_HANDOFF_MANIFEST_INVALID` diagnose negotiated handoff misuse or a bad manifest. `HOST_CLIENT_IDENTITY_MISMATCH` means one connection omitted or changed its UI correlation identity. Compatibility failures preserve the Host; correlation failures reject only the malformed client request. These are separate from Observer immutable-selector admission.
...
- Host reuse/replace keys on the one shared current Host protocol literal plus exact display `buildVersion` string equality (`stationBuildInfo().version`). Raw health alone accepts an integer protocol so generic ensure can report a mismatch; inspected/private and public preflight schemas reject every non-current value directly. Immutable lowercase-64-hex `buildIdentity` establishes recovery/update artifact identity but is not added to health and does not replace display-version compatibility. Same display version ⇒ reuse (handoff refused as unnecessary). Different display versions + matching protocol ⇒ replace (handoff only when opted in). Protocol skew or missing/malformed health, inventory, identity, or canonical lifetime evidence ⇒ refuse without a `host.list` fallback. The successor process form follows whoever requests the handoff (`bun hostMain.ts` from source CLI, `<stn> __station-host` from a binary).
...
- Never kill a version-mismatched host or remove its socket until a matching build proves that its strict recovery inventory is empty. Reopen with the build named by the error to finish or explicitly close live terminals, then retry; current-protocol idle hosts replace themselves automatically. Inspection itself grants no pin, signal, handoff, repair, or mutation authority, and its private endpoint is deliberately absent from public update facts; any later mutator must rebind its command endpoint and revalidate the relevant lifetime before acting.
