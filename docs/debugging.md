...
3. The required startup matrix is: no initial prompt through usable native input, an initial prompt through completion, and failure or exit before the trusted ready edge. Record CLI version, intervening trust/auth/model/setup UI, native order, normalized `signal`/status/turn, final snapshot, attention, and readiness. Also capture clarifying questions, plan approval, standalone questions, permissions, answers, aborts, completion, and compaction.
4. Startup lifecycle and turn completion are separate: `signalKind: session_started` may accompany `starting` or plain `idle`, but never `turn_completed` or `ready_to_read`.
