...
UI effects rather than emitting a mixed pair. A directly launched source
renderer falls back to its own verified built selector. The renderer fixes that
selector when it creates its Observer client; each later operation checks the
socket owner on the same connection without running Git or hashing source from
the UI. Station Host separately compares Host protocol and Station display build
version rather than the Observer's immutable selector. The CLI sets `STATION_TUI_PERSISTENT=1` when the
