...
The public candidate is experimental pre-alpha `v0.0.0-pre-alpha.3`. The old
...
1. Enable GitHub immutable releases, confirm the release commit is on `main`
   with `package.json` and runtime reporting at `0.0.0-pre-alpha.3`, then create
   and push `v0.0.0-pre-alpha.3`.
2. Confirm every release job passed and the successful run contains exactly one
   `accepted-release-candidate-0.0.0-pre-alpha.3-attempt-*` artifact.
...
5. Dispatch `promote-release.yml` with the successful release run ID, tag
   `v0.0.0-pre-alpha.3`, and the manual-acceptance confirmation. It rechecks the
