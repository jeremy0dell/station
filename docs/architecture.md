...
- Host retains complete transformed output plus every production-geometry transition within its bounded raw replay budget. Attach returns those ordered events while complete; after eviction it returns restoration VT from xterm's serializer plus the small set of Station-relevant modes that serializer omits. Semantic capture fails closed between xterm parser boundaries; the client retries that transient state after later output can complete the sequence. Live resize frames remain ordered with later output.
...
- Pane liveness is split from pane layout and attachment availability. On a UI restart while the host survives, panes **warm-reattach** through the Host's boundary-captured replay; on a cold start (reboot or host down) the saved layout spec **cold-respawns** fresh shells in their saved working directory. Snapshot, compatibility, and exhausted-transport failures mark the pane attachment unavailable without reporting a process exit to Observer. Layout persists to `<state_dir>/station/layout.json` (override `STATION_LAYOUT_PATH`), which deliberately does not fall back to `XDG_RUNTIME_DIR` so it survives a reboot.
