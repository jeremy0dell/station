...
- Host retains complete transformed output plus every production-geometry transition within its bounded raw replay budget. Attach returns those ordered events while complete; after eviction it returns self-contained restoration VT serialized from Host's bounded semantic terminal model at current geometry. Live resize frames remain ordered with later output.
