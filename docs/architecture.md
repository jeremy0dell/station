...
- Pane liveness is split from pane layout and attachment availability. On a UI restart while the host survives, panes **warm-reattach** through the Host's boundary-captured replay or explicit live reset; on a cold start (reboot or host down) the saved layout spec **cold-respawns** fresh shells in their saved working directory. Compatibility and exhausted-transport failures mark the pane attachment unavailable without reporting a process exit to Observer; inability to reconstruct historical output does not block a live attachment. Layout persists to `<state_dir>/station/layout.json` (override `STATION_LAYOUT_PATH`), which deliberately does not fall back to `XDG_RUNTIME_DIR` so it survives a reboot.
