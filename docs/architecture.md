...
- Update orchestration remains in the CLI. It captures one read-only aggregate and derives a capability-driven plan before mutation. Same-artifact updates revalidate the selected artifact before running ordered hook, exact Observer, Host, parked-bridge, persisted-state, and final-inspection capabilities in process. Artifact-changing updates cross once into the installed target launcher through a strict hidden successor request and authenticated encrypted receipt; a scope digest lets the successor prove the same installation owner without receiving path or command authority. Final inspection brackets the fresh runtime aggregate with local installed-artifact reads and never replans a remote target. The encrypted receipt carries exact parked-terminal preservation evidence that the public report omits.
- `stn update --reap` is the explicit destructive update path. A private digest commits the public plan, fresh preflight evidence, selected artifact, installation scope, exact runtime identities, terminal lifetimes, recovery selections, and process-group membership. The updater holds a dedicated state-directory lock, repeats those reads, and sends `SIGTERM` only to exact Host-child process-group leaders whose identity and complete membership still match. It sends `SIGKILL` only when every remaining member is unchanged and was authorized before `SIGTERM`. A mode-0600 journal records ordered phases before the first signal. An artifact-changing successor atomically takes ownership of the same lock through a private token while the public request carries only the opaque journal ID. The public v6 report contains aliases, outcomes, resume dispositions, unresolved state, and recovery commands, but no PID, process group, endpoint, path, recovery handle, or executable authority.
- `stn repair` is the explicit exceptional-recovery boundary. Inventory and action previews are read-only. Apply requires `--yes --expect-plan <sha256>`, repeats the complete inventory under a claimed repair lock, and rejects changed evidence before mutation. Terminal repair delegates to the update reaper for one exact terminal and also holds the update-reap lock. Observer cleanup delegates only to the stale-evidence policy. Recovery resume and exact-handle prune require the Observer to verify the active private journal, audit, and SQLite-consistent backup before it mutates state. Resume re-resolves the canonical handle immediately before terminal creation. Private mode-0600 journals and audit records support restart recovery and finalize interrupted attempts. Public results retain Station selectors, digests, backup and transaction IDs, typed errors, and executable recovery commands, but omit filesystem paths, process identity, provider-native targets, prompts, and transcripts.
