...
- Host retains complete transformed output plus every production-geometry transition within its bounded raw replay budget. Attach returns those ordered events while complete; after eviction it prefers restoration VT from xterm's serializer plus the small set of Station-relevant modes that serializer omits. Exact semantic recovery samples a content-free hard/soft row-copy sidecar at the same queue boundary because xterm serialization cannot retain zero-width custom OSC history. Protocol variants carry serialized VT and that sidecar as separate required fields, so Station parses VT to idle before restoration without admitting missing, duplicated, out-of-order, or hard-row-with-separator states. Semantic capture fails closed between xterm parser boundaries, and the client retries that transient state after later output can complete the sequence. At a safe boundary where exact reconstruction is unavailable, Host retains the live sink and returns RIS-prefixed, control-only reset data that restores the boundary-captured interaction modes without historical content; Station applies it before nudging PTY geometry so the child can repaint. Live resize frames remain ordered with later output.
- Host reuse requires exact protocol and build-version equality. Host protocol 6 adds required semantic-copy sidecars for exact truncation recovery. A different-build host is replaced only through its atomic stop-if-idle operation; a host with any agent or auxiliary PTY is preserved and the upgrade fails visibly. Attach transfers typed replay payloads and live frames, never PTY handles, so Station does not claim live PTY handoff between host processes.
