...
- `packages/station-host` owns the standalone `station-station-host` daemon contract and client: a process that owns PTYs and in-memory scrollback outliving the Station UI, exposing attach/list/close over its own local socket so panes can warm-reattach. Station consumes it directly; Observer application code can reach host-backed terminal behavior only through an adapter supplied by CLI composition.
...
- Host replay records the production geometry of retained data, and live resize frames remain ordered with later output bytes, so warm reattach parses every segment at the size that produced it.
- Host reuse requires exact protocol and build-version equality. A different-build host is replaced only through its atomic stop-if-idle operation; a host with any agent or auxiliary PTY is preserved and the upgrade fails visibly. Attach transfers ordered replay events and live frames, not PTY handles, so Station does not claim live PTY handoff between host processes.
