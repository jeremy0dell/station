...
- `packages/client` owns the framework-neutral rich-client observer runtime: one canonical snapshot/connection state source, the event subscription/reconnect loop, event-to-snapshot reduction, and a convergence-safe Observer service whose loads and reconciliation commit to that same reducer base before resolving to Station UI consumers.
