...
- Setup orchestration remains in the CLI: it validates and normalizes external facts, projects semantic plans into a typed presentation view with `@station/setup-messages` references, derives the existing CLI compatibility schema from that view, and implements setup-core driven ports with config, provider, process, filesystem, and Observer adapters. The text presenter owns terminal layout, styling, shell quoting, progress, and output writing; compatibility action commands and data remain JSON-only when a semantic binding exists. Provider tracking runs in-process and only sanitized commit evidence returns to setup-core. `@station/setup-core` imports no message IDs, copy, resolver, or runtime concerns. Setup copy changes and translations start in the setup-messages catalog: add or update the typed message definition and terminal variant, then let presenters select the target variant; future graphical variants live beside terminal copy rather than in setup state or flow control.
...
- Pane liveness is split from pane layout and attachment availability. On a UI restart while the host survives, panes **warm-reattach** through the Host's boundary-captured replay or mode-restoring live reset; on a cold start (reboot or host down) the saved layout spec **cold-respawns** fresh shells in their saved working directory. Compatibility and exhausted-transport failures mark the pane attachment unavailable without reporting a process exit to Observer; inability to reconstruct historical output does not block a live attachment. Layout persists to `<state_dir>/station/layout.json` (override `STATION_LAYOUT_PATH`), which deliberately does not fall back to `XDG_RUNTIME_DIR` so it survives a reboot.
