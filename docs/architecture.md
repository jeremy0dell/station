...
- Host removes OSC 9 payload bytes before output compatibility, replay retention, semantic modeling, or live delivery. It retains only the latest content-free notification ID and original timestamp per live PTY; attach captures that edge after registering the live sink, and Station emits it only after replay with ID deduplication across the acknowledgement/live race. Local fallback PTYs apply the same message-free boundary in their Station VT screen but do not gain Host durability.
