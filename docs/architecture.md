...
- `packages/contracts` owns shared application schemas and types, including `ObserverApi`, external-launch values, commands, events, snapshots, observations, provider ports, hooks, diagnostics, repair evidence, and safe errors.
...
- Observer repair inventories are point-in-time retained-session and redacted recovery-handle evidence, not recovery eligibility or persistence-mutation authority.
...
- Observer repair inspection projects provider-neutral retained-session and redacted recovery-handle evidence from one coherent persistence read. It neither evaluates recovery eligibility nor reconciles or writes durable state.
