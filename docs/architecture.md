...
- `apps/cli` owns the `stn` command surface: observer lifecycle, setup/doctor, reconcile/snapshot, hooks, debug trace, debug bundles, read-only repair inventory and previews, and terminal UI entrypoints.
...
- `packages/contracts` owns shared application schemas and types, including `ObserverApi`, external-launch values, commands, events, snapshots, observations, provider ports, hooks, diagnostics, repair evidence/plans/audits, and safe errors.
...
- Repair inventories and dry-run plans are point-in-time evidence, not signal, PTY-close, or persistence-mutation authority. A future executor must capture a fresh inventory and exactly revalidate every socket, process, PTY instance, and topology identity before acting.
