...
| `StationSnapshot` | Current normalized graph held in memory. `rows` is configured worktree inventory; `sessions` is canonical session membership; and required `sessionGroups` carries normalized organizational state for configured projects. Reconcile replaces the base projection; recorded Group mutations refresh only the Group projection through the same serialized writer, and accepted harness reports can project status and readiness between reconciles. It is derived and not a durable replay log. |
...
project. They validate configured-project and canonical-session identity before
calling `SessionGroupStore`, then refresh only the durable Group projection. Changed
Group events are persisted and published in canonical order before command success;
validated no-ops emit no Group event. This path does not read providers or publish
`observer.reconciled`.
...
Observer core serializes full reconciles, Group-only durable projection refreshes,
completed provider-health commits, and harness-report authorization plus base snapshot
projection on one non-poisoning writer chain. A Group-only refresh prunes against the current
canonical sessions and never invokes providers. A health commit persists one observation, coherently updates the
...
| Snapshot writer ordering | Full reconciles, Group-only durable projection refreshes, and harness-report authorization plus base projection share a non-poisoning promise chain. Group refresh never scans providers or publishes a reconcile event. Readiness persistence revalidates the live snapshot after its write. Scheduled reconcile requests coalesce; queued work after a run receives a later flush. |
