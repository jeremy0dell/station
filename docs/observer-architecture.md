...
| `apps/cli/src/commands/update.ts`, `apps/cli/src/update/updateConvergenceUseCase.ts`, update ports/adapters, and `apps/cli/src/commands/registry/update.ts` | update argument translation, convergence orchestration, strict child transport, public report projection, and default wiring | The command is a driving adapter over the update convergence port. The use case depends only on deterministic policies and purpose-owned driven ports. Runtime children, pinned successor JSON transport, and public-report projection are outbound adapters selected by the update registry composition root. |
