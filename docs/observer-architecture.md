...
| Update plan freshness | Each plan binds the selected target and the current public/private aggregate. Safe #640 actions record the exact executed digest and end with a fresh aggregate and plan. The initial changed-artifact plan remains eligible for #641 only when it is the newest no-mutation `reap-required` plan with no later audit; #641 reruns and compares it before adding exact Station-owned process-group authorization. |
