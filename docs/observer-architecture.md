...
| Observer process identity | Driven | `ObserverProcessIdentityEvidenceSource` | bounded local `ps`/`lsof`/`/proc` process-evidence adapter | One shared read-only verifier compares executable provenance, exact argv, OS start token, per-launch token, build selector, and resolved socket. It distinguishes exact identity, a narrowly proven `installed-path-replaced` live image, mismatch, and unavailable evidence; missing or ambiguous records never become replacement evidence. Handoff, stale-evidence repair, and equivalent reap checks consume this verifier; no parallel weaker verifier exists. |
...
| `apps/cli/src/commands/update.ts`, `apps/cli/src/update/updateConvergenceUseCase.ts`, update ports/adapters, and `apps/cli/src/commands/registry/update.ts` | update argument translation, convergence orchestration, typed Host inspection/convergence, strict child transport, public report projection, and default wiring | The command is a driving adapter over the update convergence port. The use case depends only on deterministic policies and purpose-owned driven ports. Direct Host runtime, remaining runtime children, pinned successor JSON transport, and public-report projection are outbound adapters selected by the update registry composition root. |
...
| State | Authority and lifetime |
| --- | --- |
| Update child authorization transport | Observer mutation receives the exact planned owner-or-absent tuple, selected recovery handles, target selector, digest, and one nonce through a bounded one-shot inherited pipe. The payload never enters argv, environment, files, public reports, logs, diagnostics, or recovery commands; the child closes its descriptor before descendant spawn, independently re-inspects the same aggregate immediately before mutation, and constrains lifecycle stop/start to the planned owner. |
| Update Host predecessor identity | Recovery inventory, same-connection list fallback, and the selected lifecycle mutation carry the exact committed incumbent protocol/build identity. The Host binds the first non-health identity and refuses changes on that connection; only reconnect after old Host closure mints the configured target identity. Ordinary ensure calls keep their existing shared-client lifecycle, while update convergence owns and disposes every per-call client from its factory. |
...
| Update aggregate inspection | One read-only CLI-owned aggregation of Observer identity/health/recovery, Host build/inventory, provider hook health, and terminal dispositions for one installed and selected artifact. One required port constructs the public aggregate and private sidecar together, then strict CLI-owned validation requires matching Observer build selectors and one unique exact opaque handle for every public selected-handle session before projecting public-only #665 evidence. Exact executable provenance is normally required. Atomic installation replacement has one narrower evidence state: exact parsed argv, pidfile, health, OS start, process token, build selector, and socket remain pinned while only the old live image differs from the currently installed path identity, so only the existing explicit same-or-higher-build cooperative restart is eligible. Missing or ambiguous process records, executable or argv mismatch, lower builds, automatic signals, and destructive recovery still refuse. Public evidence is redaction-safe; its private sidecar retains the Observer PID, OS start, process token, build selector, configured socket, and selected opaque Station recovery-handle IDs through immediate pre-mutation reinspection. A changed tuple or selected handle makes the inspected plan stale without starting or restarting anything. Neither form grants mutation authority. |
| Update Observer convergence | The CLI use case revalidates the complete private Observer and selected-handle commitment immediately before invoking its purpose-named runtime adapter. The child mutation is pinned to the selected immutable build and configured socket, accepts only exit zero plus strict healthy running evidence with complete PID, OS start, socket, and version identity, and rejects every contradiction before Host or reconcile mutation. Typed lifecycle failure is exit one only; its sanitized cause and bounded startup evidence may appear only on an `observer-convergence` execution failure. Automatic process signals remain outside this boundary. |
| Update convergence plan | A deterministic provider-neutral decision over one aggregate inspection, explicit selected target, and exact install owner/action plus manager argv when applicable. The evaluator (`incumbent-cli` or `successor-cli`) is separate from target ownership. Observer build relation and singleton replacement admission are distinct evidence: the existing singleton classifier must return `candidate-wins` before a different installed Observer may restart, while an incumbent win or refusal produces a non-mutating `singleton-refused` block. A selected build not installed yet produces successor reinspection rather than incumbent restart authority; exact matching unhealthy Observers retain their explicit restart path. The single current aggregate carries admission directly in the update report and its digest. Successor evaluators use the install owner's local exact-target proof, not ordinary latest discovery. Their transport admits one strict current JSON result only when the selected channel, exact no-op install owner and manager argv, top-level and nested artifact facts, `successor-cli` evaluator, consistent known immutable build identity, allowed non-preview result kind, and derived process exit retain the inherited pin; all contradictions remain successor-boundary failures. The shared strict refinement also cross-checks every plan's actions, reasons, terminal counts, recovery decision, status, and phases against its aggregate. A successful runtime audit contains every executable phase and provider exactly once, in canonical order, with completed status; artifact application audits its exact typed install commitment, while failed or skipped actions belong only to the final exact stage of `execution-failed`. Public install commitments, phases, component decisions, and digest are serialized in the current report; a completed terminal-handoff audit additionally carries only the canonical immutable PTY lifetime receipt. Handoff fidelity (`processes` or `screen`) is an exact Host-and-terminal plan fact bound by the digest, constrained request, typed receipt, and every success or failure audit; requested and acknowledged fidelity must match. The registry-composed Host adapter supplies the single aggregate inspection directly and executes either a constrained idle-replace operation or a constrained handoff operation with the exact incumbent build/protocol, selected target build, and canonical immutable inventory; CLI presentation owns neither path. Lifecycle-only read calls validate the old Host on the same persistent connection as the selected mutation without making ordinary inventory calls compatible with the target. A disappeared incumbent returns `absent` or `stale` without spawn, an exact target build with the exact inventory returns `already-converged` without mutation, and any other build or inventory returns `stale`; no executor fallback may change the planned action. Only a completed typed receipt echoes the commitment, actual `ensuredBy`, and acknowledged fidelity. Every other outcome returns to fresh aggregate inspection before success or failure is reported; concurrent exact convergence is retained as fresh converged evidence in a superseded execution failure rather than mislabeled as this executor's success. Process tokens, recovery-handle IDs, aggregate paths, raw provider data, and process groups are not serialized. `--dry-run --reap` projects only ownership-classified public terminal/session dispositions and their recovery consequences; it grants no authority and executes nothing. Final verification after handoff requires the same exact terminal set; an absent terminal needs an explicit evidenced disposition and is never inferred to have exited naturally. A verified no-op plan is the sole source of `current`. A typed public-report adapter sanitizes every nested SafeError after child parsing and defensively before JSON or text presentation. Non-mutating text shows all seven result-phase statuses and per-terminal consequences, and one terminal-safe encoder protects every untrusted rendered string. The separate `execution-failed` diagnostic envelope preserves only a strict sanitized Observer lifecycle cause and bounded startup evidence, including through a successor report. |
| Update terminal handoff identity | The canonical convergence commitment and typed handoff receipt use the session-bound `{ terminalTargetId, ptyId, ptyInstanceId, sessionId }` identity. Recovery ownership belongs to the Station session, so a receipt or final inventory that reassigns an otherwise unchanged PTY lifetime to another session is stale and cannot prove convergence. |
| Update convergence digest | Versioned SHA-256 commitment to the exact install owner/action and manager argv, authorization-relevant target, handoff fidelity, session-bound aggregate, private ownership, and selected Station-handle facts under an explicit canonical encoding that rejects ill-formed Unicode before UTF-8 hashing. It reveals no private value, is not authority, and never substitutes for #641's fresh exact process-group authorization and journal. |
...
| Update plan freshness | Each plan binds the selected target, install owner/action and manager argv, and current public/private aggregate. Safe #640 actions audit the exact typed install or runtime action, record the exact executed digest, and end with a fresh aggregate and plan. `--dry-run --reap` projects only public Station-owned terminal/session dispositions and recovery consequences; it neither inventories process groups nor authorizes reaping. The initial changed-artifact plan remains eligible for #641 only when it is the newest no-mutation `reap-required` plan with no later audit; #641 reruns and compares it before adding exact Station-owned process-group authorization. |
