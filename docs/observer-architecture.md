...
Update convergence is a CLI-owned composition over these capabilities. Its read-only aggregate retains only invocation-local commitments; the public v6 report contains the initial and final aggregates, the derived plan, ordered provider-neutral hook results, and aliased reaping and session-resume outcomes. Same-artifact updates revalidate the installed artifact before they execute hook, exact Observer, Host, parked-bridge, persisted-state, and final-inspection capabilities in the current process. Artifact-changing updates cross once into the installed target launcher with a strict request containing channel, artifact, an installation-scope digest, handoff policy, sorted hook providers, and an optional opaque journal ID. The digest commits to stable local ownership fields without carrying a path or executable command. The target performs runtime capabilities in process and returns a bounded, authenticated encrypted receipt that includes exact parked-terminal preservation evidence. The target consumes the one-shot transport key before composing runtime capabilities, and the parent validates target, channel, installation scope, provider, plan derivation, terminal preservation, and exit correlation before one public projection. Final inspection brackets the runtime aggregate with local installed-artifact reads without resolving another remote target. No successor receipt, parked-terminal identity, endpoint, PID, process group, recovery handle, journal content, or updater-only Host command is public authority.

`stn update --reap` derives separate private authorization from the public plan, private preflight commitments, selected artifact and installation scope, exact Observer and Host identities, terminal lifetimes, selected recovery identities, and complete POSIX process-group membership. Aggregate `evidenceComplete` remains unchanged when unrelated retained sessions lack worktree evidence. Private reap authorization first matches the entire public recovery assessment against its private source and verifies parked commitment coverage. Its exclusion policy admits only missing-worktree uncertainty with no session or project/worktree identity overlap with any Host terminal, including dead and auxiliary terminals, or any parked terminal. The contract completeness check accepts those session IDs for that invocation while preserving every other completeness rule and the full-inventory schema invariant. The digest commits to the original public aggregate, exact Observer and Host identities, parked commitments, and selected recovery handle IDs in targets. Private recovery assessments are validated but are not independently digested. Typed refusals retain a specific SafeError cause in the existing public error fields. It acquires a dedicated update lock and repeats the full inventory before it persists authorization. Only an exact Host child that is also its process-group leader may receive `SIGTERM`. `SIGKILL` follows three seconds later only when every remaining PID, process group, and start token is an exact subset of the authorized group. A surviving member may have a new parent only after the authorized group leader exits, because POSIX reparents the child at that point; new, otherwise changed, or unreadable evidence stays unresolved. A mode-0600 journal records the transaction from `authorized` through `completed`. Before `reap-started`, changed evidence invalidates authorization. After `reap-started`, retries continue or refuse that journal and cannot report `current` until the transaction completes. An artifact-changing successor atomically takes ownership of the same lock through a one-shot private token; the strict request retains only the opaque journal ID. The updater proves the incumbent Host has no PTYs before artifact or runtime crossover and uses the existing explicit recovery-handle selector and `session.resumeAgent` command with an exact expected session/provider identity. It verifies the resulting live Station session before recording resume completion.

`stn repair` composes exceptional recovery in the CLI without creating another lifecycle planner, signal path, or stale-evidence policy. Its public version-1 inventory combines independently settled runtime and recovery sections and commits the configured state scope. Preview plans have `authorization: "none"`. Apply requires both consent flags, takes the repair lock, repeats the inventory, and matches the plan digest. Exact terminal reap takes the update-reap lock second and delegates one target to its authorizer, journaled process-group executor, escalation rules, and postcondition. Observer cleanup delegates exclusively to the Observer singleton stale-evidence policy.

Recovery repair reads the existing redacted recovery assessment from one SQLite snapshot and derives a canonical digest without provider-native targets or local paths. Resume and prune recheck that digest inside the serialized Observer use case. Resume re-reads the coherent snapshot and resolves the same canonical handle immediately before terminal creation. Prune deletes only the selected eligible handle and verifies that every unrelated handle is unchanged; it may remove the sole handle only through this explicit command. Automatic recovery selection remains unchanged. Before terminal, resume, or prune mutation, the CLI uses SQLite's online backup API, prepares only the new owner-only copy in DELETE journal mode, reopens it read-only, and verifies its size, schema, integrity, recovery digest, and content digest. The recovery Observer command independently verifies that its active private journal and audit name that exact backup and action, so direct command dispatch cannot bypass those records. The result reports only the opaque backup ID and digests. A private atomic audit is written before other mutation, rediscovered after interruption, and finalized with a terminal status. A private restart journal advances through explicit commit phases. An interrupted resume never launches again without a verified completed postcondition; idempotent terminal, cleanup, and prune actions may continue the same committed journal.
...
opened or a known migration cannot be applied. Initialization closes its connection
on pragma or migration failure and retains the typed stage and original cause. Application code must not catch
...
The `AgentExecutionProvider` driven port separates remote agent lifetime from
local terminal presentation. Session commands preserve execution placement and
retain uncertain cloud launches. Reconcile consumes provider-neutral execution
and agent status. The E2B adapter owns its SDK, credentials, durable sandbox
identity, private terminal broker, source transfer, and verified result files.
