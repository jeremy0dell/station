...
```text
Install experimental Station v0.0.0-pre-alpha.6 and validate setup on this machine.

Safety and scope:
- Do not clone the repository or build from source. Use only
  `https://github.com/jeremy0dell/station/releases/download/v0.0.0-pre-alpha.6/install.sh`.
...
The released `install.sh` is stamped with `v0.0.0-pre-alpha.6`. With no
...
```bash
curl --disable -fsSL https://github.com/jeremy0dell/station/releases/download/v0.0.0-pre-alpha.6/install.sh | \
