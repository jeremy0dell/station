...
- The welcome intro is seeded only at store creation, and `bun --hot` preserves the store across reloads, so it appears only on a true cold boot, not a hot reload. Set `[workspace].welcome_on_boot = false` in `config.toml` to disable it.
...
- Non-UTF-8 / raw-byte PTY output is corrupted: the node-pty bridge decodes as UTF-8, so invalid bytes become the replacement character (`�`) before Station sees them. This affects legacy-encoded TUIs and `cat`-ing binary data. A binary-safe transport is not yet implemented.
